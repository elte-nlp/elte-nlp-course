 ...
 \begin{slide}[toc=]{IOB tagging cont.}
  ...
  \begin{slide}[toc=MEMMs]{Maximum entropy Markov models (MEMMs)}
    ...
    \begin{small}
      ...
      $$
    \end{small}%
    How can $Y_m$ depend on $\mathbf{X}$ then? The trick is in how $O_m$ is defined.
  \end{slide}

  \begin{slide}[toc=]{Feature functions}
    The $Y_{m-1},O_m$ pair is defined as $\mathbf{f}(y_k,\mathbf{x}, m)$,
    where $f(\cdot)$ is a function that produces a feature vector at $m$ based
    on the value $Y_{m-1}=y_k$ and $x$.\medskip

    In NLP, we condition only on \emph{local features} within a
    \emph{context window} around the element to be tagged. A few example
    features for POS tagging, designed by linguists:
    ...
    \begin{small}
      ...
      \item the POS tag $y_k$ of the previous element.
      \end{itemize}
    \end{small}
  ...
  \begin{slide}[toc=]{Maximum entropy Markov models (MEMMs)}
    ...
    \begin{small}
      ...
      P(Y_m = y_i|Y_{m-1}=y_k,\mathbf{x})=\frac{\exp (\mathbf{w}_i \cdot \mathbf{f}(y_k,
        ...
        \mathbf{x}, m))},
      $$
      where each $\mathbf{w}_i$ is a weight vector for the $y_i\in Y$
      label.\bigskip
    \end{small}
  ...
  \begin{slide}[toc=Label bias]{Label bias}
    ...
    \emph{\href{https://awni.github.io/label-bias/}{\gold label bias}} problem:
    the model cannot easily recover from past labelling mistakes made in a low
    confidence situation.
  \end{slide}

  \begin{slide}[toc=]{Label bias -- example}
    \begin{small}
      A POS tagger tags the sentence ``\textit{cat sat}'' as
      \texttt{\footnotesize ARTICLE VERB}, as
      \begin{itemize}
        \item The tagger cannot
          recover from the skewed posterior distribution of \textit{cat} at
          \texttt{\footnotesize <S>} with \textbf{\gold local} normalization
          (left).
        \item The unnormalized $\mathbf{w}_i \cdot \mathbf{f}(\cdot)$
          observations (right) reveal that
          \begin{enumerate}
            \item the tagger is not confident about ``\textit{cat}'' starting a
              sentence,
            \item \textbf{\gold globally} \texttt{\footnotesize NOUN VERB} has
              a higher score ($\log$ so summed along the edges).
          \end{enumerate}
      \end{itemize}
    \end{small}
    ...
    \begin{center}
      ...
      \footnotesize(Example from the \href{https://awni.github.io/label-bias/}{Awni Hannun -- Writing About Machine Learning} blog.)
    \end{center}
  \end{slide}

  \begin{slide}[toc=CRFs]{Conditional Random Fields cont.}
    Linear chain CRFs are discriminative models designed to avoid the label
    bias. They assume the following \emph{undirected} structure:
    \begin{center}
      \begin{tikzpicture}[ampersand replacement=\&]
        \matrix[matrix of math nodes,column sep=2em,row sep=3em] (m) {
          Y_1 \& Y_2 \& Y_3 \& \cdots \& Y_{n}\\
  ...
  \begin{slide}[toc=]{Conditional Random Fields cont.}
    Somewhat similarly to MEMMs, the $\phi_m(\cdot)$ \emph{\gold potential
    functions} are modeled linearly using a feature function and a
    corresponding weight vector. They are basically the nominators of the
    softmax.
    $$
    \phi_m(y_m, y_{m+1},\mathbf{x})={\exp (\mathbf{w} \cdot
      \mathbf{f}(y_m,y_{m+1}, \mathbf{x}, m))}.
    $$
    The crucial difference is that the normalization is \emph{global}:
    $$
    P(\mathbf{y}~|~\mathbf{x}) =
