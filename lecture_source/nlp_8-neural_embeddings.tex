...
\begin{slide}[toc=Intrinsic]{Intrinsic evaluation}
  ...
  \begin{enumerate}
    \item \emph{\gold Word similarity}
    \item \emph{\gold Analogy}
  \end{enumerate}
\end{slide}

\begin{slide}[toc=]{Intrinsic evaluation -- Similarity}
  Similar words should have similar vectors.
  \begin{itemize}
    \item semantic: dog -- hound
    \item grammatical: dog\textbf{s} -- pear\textbf{s}
  \end{itemize}\bigskip

  The similarity of two words in measured by the
  \emph{\gold cosine similarity} of their representations:
  $\frac{E(w_1)\cdot E(w_2)}{\|E(w_1)\|\times \|E(w_2)\|}$.\bigskip

  \begin{small}
    Note: normalization is important as the vectors' length is roughly
    proportional to the logarithm of the word's frequency
    \citep{arora2016latent}.
  \end{small}
\end{slide}

\begin{slide}[toc=]{Intrinsic evaluation -- Similarity cont.}
  Similarity can be nicely visualized using dimension reduction
...
\begin{slide}[toc=]{Intrinsic evaluation -- Analogy}
  Analogy tasks test how well \emph{\gold relationships}
  between words, e.g,. $king:queen$ and $man:woman$ correspond to similar
  geometric relationships between their vectors.\bigskip

  The relationships correspond to the vector \emph{difference}, i.e.
  $E(king)-E(queen)\approx E(man)-E(woman)$\bigskip

  Or, in a slightly different formulation, whether the word whose embedding is
  the closest to $E(king)-E(queen) + E(woman)$ is $man$.
\end{slide}

\begin{slide}[toc=]{Intrinsic evaluation -- Analogy cont.}
  Examples for semantic and syntactic analogies:
  \begin{center}
    \includegraphics[width=0.8\textwidth]{figures/analogy.eps}
    
    (\footnotesize Figure from \citet{mikolov2013linguistic}.)
  \end{center}  
\end{slide}

\begin{slide}[toc=]{Intrinsic evaluation -- Datasets}
  There are also purpose built \emph{datasets} for intrinsic evaluation, e.g.,
  \begin{itemize}
  \item the
    \href{http://alfonseca.org/eng/research/wordsim353.html}{WordSim-353}
    dataset containing 353 English word pairs with semantic similarity scores
    between 0.0 and 10.0. (Note: the original dataset confused \emph{similarity}
    with \emph{relatedness}; the linked version mostly fixes this.)
  \item \href{https://fh295.github.io/simlex.html}{SimLex-999} superceded
    WordSim-353 with 999 word pairs with a similarity score on the same scale.
...
\begin{slide}{References}
  ...
  \begin{footnotesize}
    ...
    \bibentry{arora2016latent}\medskip
...
\begin{slide}[toc=]{References cont.}
  \begin{footnotesize}
    ...
    \bibentry{mikolov2013linguistic}\medskip
